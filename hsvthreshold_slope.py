...
def corners(pts, img):
	...
	if window % 2 == 0:
		window += 1
	medianed = scipy.signal.medfilt(slopes, window)
	mprev = np.roll(medianed, 1)
	mnext = np.roll(medianed, -1)
	mderiv = np.abs(mnext - mprev / (next[:,3] - prev[:,3]))
	...
	for i in range(l):
		...
		print "%f\t%f" % (sorted[i][3], slopes[i])
		side.append(sorted[i][:2] + [cx, cy])
		for k in range(1, 15):
			next = (i + k) % l
			prev = (i - k) % l
			if not (mderiv[i] > mderiv[next] and mderiv[i] > mderiv[prev]):
				perfect = False
